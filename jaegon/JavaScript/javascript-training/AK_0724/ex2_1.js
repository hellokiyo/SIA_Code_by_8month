...
console.log(`names 변수상자의 배열인가요? : ${Array.isArray(names)}`);
console.log(`names 변수상자의 배열인가요? : ${Array.isArray(fish1)}`);
...
console.log(`fishes 변수상자에 들어있는 칸의 갯수 : ${fishes.length}`);
...
console.log(`fish1 물고기의 이름 : ${fish1.name}`); //아래줄이랑 똑같은 코드임
console.log(`fish1 물고기의 이름 : ${fish1['name']}`); //방식이 어려움

let attrName = 'name';
console.log(`fish1 물고기의 이름 : ${fish1[attrName]}`);

console.log(`집 안에 있는 첫번째 아이가 가지고 있는 첫번째 물고기의 나이는? : ${house['children'][0]['fishes'][0]['age']}`)
// console.log(`집 안에 있는 첫번째 아이가 가지고 있는 첫번째 물고기의 나이는? : ${house.children[0].fishes?[1]}`);


//붕어빵(객체) 안에 들어있는ㄴ 속성들을 글자로 바꿔주기
